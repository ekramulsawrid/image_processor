...
HW_resize(ImagePtr I1, int ww, int hh, int kernelID, double param, ImagePtr I2)
{
	...
	int t;
	ChannelPtr<uchar> src, dst, inter;
	for (int ch = 0; IP_getChannel(II, ch, src, t); ch++) {
		IP_getChannel(I2, ch, dst, t);
		IP_getChannel(I3, ch, inter, t);

		// horizontal pass scales rows
		for (int y = 0; y < h; ++y) {

			// PUT YOUR CODE HERE
			resize1D(src + y * w, w, ww, 1, kernelID, param, inter + y * ww);
		}
		...
		for (int x = 0; x < ww; ++x) {
			...
			// PUT YOUR CODE HERE
			resize1D(inter + x, h, hh, ww, kernelID, param, dst + x);
		}
	}
